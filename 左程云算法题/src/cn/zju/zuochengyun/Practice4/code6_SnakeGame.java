 ...
 * 最开始的时候小Q操纵―条长度为0的蛇蛇从矩阵最左侧任选一个单元格进入地图，蛇每次只能够到达当前位置的右上相邻，右侧相邻和右下相邻的单元格。
 * 蛇蛇到达一个单元格后，自身的长度会瞬间茄上该单元格的数值，任何情况下长度为负则游戏结束。
 * 小0是个天才他拥有一个超能力，可以在游戏并始的时候把地图中的某一个节点的值变为其相反数(注:最多只能改变一个节点)。
 * 问在小○游戏过程中，他的蛇蛇最长长度可以到多少?
 * 比如:
 * 1 -4 10
 * 3 -2 -1
 * 2 -1 0
