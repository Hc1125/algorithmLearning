 ...
 * 可以从board任何位置出发，每一步可以走向上下左右四个方向
 * 但是一条路径已经走过的位置，不能重复走
 * 返回words哪些单次可以被走出来
 * 例子：
 * board=
...
public class code8_WordSearch {
    ...
    public static int process(
        ...
        if (cha == 0) {  // 如果这个row col位置是之前走过的位置
            return 0;
        }
        int index = cha - 'a';
        ...
        int fix = 0;   // 从row和col位置出发，后续一共搞定了多少个答案
